...
public class ReportingServiceImpl implements ReportingService {
    ...
    Map.Entry<String , BigDecimal> entry =   trxList.stream().filter(trx -> trx.getTrxAmount().compareTo(BigDecimal.ZERO) == -1)
             ...
             .entrySet().stream().min(Comparator.comparing(Map.Entry::getValue)).get();
